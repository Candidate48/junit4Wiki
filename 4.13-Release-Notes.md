...
### [Pull request #1406 (part 1)](https://github.com/junit-team/junit4/pull/1406): Improve error message when TemporaryFolder.newFolder(String) fails
When `newFolder(String path)` was not able to create the folder then it always failed with the error message "a folder with the name '<path>' already exists" although the reason for the failure could be something else. This message is now only used if the folder really exists. In all other cases it fails now with the message "could not create a folder with the path '<path>'"
...
### [Pull request #1421](https://github.com/junit-team/junit4/pull/1421): ExternalResource: declare after() to throw Throwable
...
### [Pull request #1519](https://github.com/junit-team/junit4/pull/1519): Deprecate ExpectedException
...
### [Pull request #1414](https://github.com/junit-team/junit4/pull/1414): Non-static fields don't shadow

A field from a child class does not shadow a field from its parent class. Example:

    public class TestBase {
      @Rule
      public final LoggingRule log = new LoggingRule("base");
    ...
    public class SomeTest extends TestBase {
      @Rule
      public final LoggingRule log = new LoggingRule("someTest");

      ...
    }

The generated log for a test method looks like `base.before someTest.before test someTest.after base.after`.
