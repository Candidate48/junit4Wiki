...
The signature of `ExpectedException.expectCause()` would not allow the caller to pass in a `Matcher<Object>` (which is returned by `MoreMatchers.notNullValue()`). This was fixed by changing the method to take in a
