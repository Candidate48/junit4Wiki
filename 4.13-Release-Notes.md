...
### [Pull request #1517](https://github.com/junit-team/junit4/pull/1517): ThreadGroup instance leaked when using Timeout rule.
The `ThreadGroup` created for handling the timeout of tests is never destroyed, so the main thread group keeps a reference to all timeout groups created during the tests. This causes the `threadGroup` to remain in memory, and all of its context along with it.
