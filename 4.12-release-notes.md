...
JUnit4ClassRunner was deprecated in JUnit 4.13 (seven years and seven major releases ago) with a comment saying "This may disappear as soon as 1 April 2009". We started having some problems with running the tests in JDK 7, and we decided to delete the class and its support classes. Although we try very hard to maintain backwards compatibility, JUnit4ClassRunner didn't support Rules, it wasn't designed to be extensible, and it was in an internal package. Please use BlockJUnit4ClassRunner.

## [Pull request #817:] (https://github.com/junit-team/junit/pull/817) Support for hierarchical context hierarchies

The AnnotatedBuilder is a strategy for constructing runners for test class that have been annotated with the @RunWith annotation. All tests within this class will be executed using the runner that was specified within the annotation.
