...
Timeout exceptions try to determine if there's a child thread that's causing the problem, and if so its stack trace is included in the Exception in addition to the that of the main thread.  Feature must be enabled with a `TestRule` such as `new Timeout(100, TimeUnit.MILLISECONDS).lookForStuckThread(true)`.
