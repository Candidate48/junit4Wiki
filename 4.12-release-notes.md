...
### [Pull request #583:](https://github.com/KentBeck/junit/pull/583) ExpectedException now fails when an AssertionError is expected but not thrown

When a test expected the code under test to throw an AssertionError by using ExpectedException (together with the `ExpectedException#handleAssertionErrors` option) but the code didn't do so the test was not failing. This was due to the fact that the ExpectedException rule caught its own AssertionError issued to signal that the expected exception was not thrown. Now the test would fail as expected.

### [Pull request #511:](https://github.com/KentBeck/junit/pull/511) Maven project junit:junit:jar


_How to install the Maven_
...
Create directory _.m2_ in you _user home_. Then the artifacts and plugins are stored in `~/.m2/repository`.
( _~_ stands for user home)


_How to launch the build in command line_

Clone the [project](https://github.com/KentBeck/junit) and navigate to the project root on your local system.
Clean the previous build in _target_ directory, build the project, and install new artifacts in your local repository:

`build/maven/apache-maven-3.0.4/bin/mvn clean install`

On Windows type the command `build\maven\apache-maven-3.0.4\bin\mvn clean install`.
...
_Installing the Maven project in Eclipse_
...
_Installing the Maven project in IntelliJ IDEA_
...
On right hand side is _Maven Projects_ tab. Click on + and add _pom.xml_ into the project. Then click on the icon _Maven Settings_, and set _Maven home directory_ as the location of extracted Maven zip on your system. Click on the green triangle and launch the build.
